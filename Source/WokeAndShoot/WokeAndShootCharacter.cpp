...
void AWokeAndShootCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	//Controller Turning
	// PlayerInputComponent->BindAxis("TurnRate", this, &AWokeAndShootCharacter::TurnAtRate);
	// PlayerInputComponent->BindAxis("LookUpRate", this, &AWokeAndShootCharacter::LookUpAtRate);
}
...
void AWokeAndShootCharacter::OnFire()
{
	...
	if (World != nullptr)
	{
		//LineTrace + DebugLine if necessary
...
void AWokeAndShootCharacter::GetViewPointRotLoc(FVector &ViewPointLocation, FRotator &ViewPointRotation) 
{
...
{
	UParticleSystemComponent* TracerComponent = UGameplayStatics::SpawnEmitterAtLocation(GetWorld(),TracerParticle, HitResultLocation, ShotDirection.Rotation());
	...
	if (TracerParticle)
	{
		...
		UE_LOG(LogTemp,Warning, TEXT("%f"),OutDistance);
